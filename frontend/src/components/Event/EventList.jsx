    ...
    <div className="p-20 bg-gray-100 min-h-screen">
      ...
      <div className="flex flex-wrap gap-8 justify-center">
        {events.map((event) => (
          <div
            key={event._id}
            className="max-w-lg bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105"
          >
            <img
              src={event.imageURL}
              alt={event.title}
            ...
            <div className="p-6">
              ...
              <div className="flex gap-12 mb-4 text-gray-600 text-sm">
                <p>
                  ...
                  {event.createdBy[0].username}
                </p>
                <div>
                  <p className="flex flex-col">
                    <span className="font-semibold">Time to Live:</span>{" "}
                    {new Date(event.liveDate).toLocaleString()}
                  </p>
                </div>
              </div>
              <div className="flex justify-between  items-center mt-4">
                <div
                  className={`bg-blue-600 text-white text-sm font-semibold px-4 py-2 rounded-lg ${
                    event.liveData ? "bg-blue-700" : "bg-green-600"
                  }`}
                >
                  {event.liveData ? "Live Now" : "Upcoming"}
                </div>
                {event.createdBy[0]._id === currentUser._id ? (
                  <button
